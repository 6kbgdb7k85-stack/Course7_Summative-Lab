...
class User:
    ...
    def add_project(self,project):
        if project.id in self.projects:
            print(f"User '{self._name}' already assigned to project '{project.name}'.")
        else:
    ...
    def add_task(self,task):
        if task.id in self.tasks:
            print(f"User '{self._name}' already assigned to task '{task.title}'.")
        else:
    ...
    def remove_project(self,project):
        if project.id not in self.projects:
            print(f"User '{self._name}' not assigned to project '{project.name}'.")
            return
    ...
    def remove_task(self,task):
        if task.id not in self.tasks:
            print(f"User '{self._name}' not assigned to task '{task.title}'.")
            return
        self.tasks.remove(task.id)
        task.assign_user(-1)
        edit_data(User.TABLE,self.id,{"tasks":self.tasks})
        cprint(f"Task '{task.title}' no longer assigned to user '{self._name}'","green")
